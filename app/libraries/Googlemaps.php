...
class Googlemaps {
    ...
    var $map_types_available = []; // The other MapTypes available for selection on the map
    ...
    var $styles = []; // An array of styles used to colour aspects of the map and turn points of interest on and off
    ...
    var $markers = []; // An array used by the library to store the markers as they are produced
    var $markersInfo = []; // An array containing marker information (id, latitude, longitude etc) for use elsewhere
    var $polylines = []; // An array used by the library to store the polylines as they are produced
    var $polygons = []; // An array used by the library to store the polygons as they are produced
    var $circles = []; // An array used by the library to store the circles as they are produced
    var $rectangles = []; // An array used by the library to store the rectangles as they are produced
    var $overlays = []; // An array used by the library to store the overlays as they are produced
    ...
    var $directionsWaypointArray = []; // An array of waypoints. eg array("Boston, MA", "Times Square, NY");
    ...
    var $drawingModes = []; // An array of modes available for use. Accepted values are marker, polygon, polyline, rectangle, circle
    var $drawingOnComplete = []; // An array of JS to execute when shapes are completed, one array element per shape. For example: array('circle'=>'JS here', 'polygon'=>'JS here');
    var $drawingOnEdit = []; // An array of JS to execute when shapes are changed/resized, one array element per shape. For example: array('circle'=>'JS here', 'polygon'=>'JS here');
    ...
    var $placesTypes = []; // The types of places to search for. For a list of supported types see http://code.google.com/apis/maps/documentation/places/supported_types.html
    ...
    var $placesAutocompleteTypes = []; // The types of places for the autocomplete to return. Options can be seen here https://developers.google.com/maps/documentation/javascript/places#places_autocomplete but include 'establishment' to only return business results, '(cities)', or '(regions)'
    ...
    function add_marker($params = []) {
        $marker = [];
        $this->markersInfo['marker_' . count($this->markers)] = [];
    ...
    function add_polyline($params = []) {
        ...
        $polyline['points'] = []; // An array of latitude/longitude coordinates OR addresses, or a mixture of both. If an address is supplied the Google geocoding service will be used to return a lat/long.
    ...
    function add_polygon($params = []) {
        $polygon = [];

        $polygon['points'] = []; // The positions (latitude/longitude coordinates OR addresses) at which the polygon points will appear. NOTE: The first and last elements of the array must be the same
    ...
    function add_ground_overlay($params = []) {
        $overlay = [];
    ...
    function create_map() {
        ...
        if ((is_array($this->kmlLayerURL) && count($this->kmlLayerURL)) || (!is_array($this->kmlLayerURL) && $this->kmlLayerURL != "")) {
            if (!is_array($this->kmlLayerURL)) {
                ...
                $this->kmlLayerURL = [$this->kmlLayerURL];
            }
        ...
        if (strtolower($this->map_type) == "street") {
            // if defaulting the map to Street View
            $this->output_js_contents .= '
        ...
        if ($this->center == "auto") {
            // if wanting to center on the users location
            $this->output_js_contents .= '
        ...
        if ($this->places) {
            ...
            if ($this->placesLocationSW != "" && $this->placesLocationNE != "") {
                // if search based on bounds
            ...
            if (($placesLocationSet || $this->placesLocation != "") || count($this->placesTypes) || $this->placesName != "") {
                ...
                if ($placesLocationSet) {
                    ...
                    if ($this->placesLocation != "") {
                        // if search based on a center point
                        if ($this->is_lat_long($this->placesLocation)) {
                            // if centering the map on a lat/long
        ...
        if ($this->jsfile == "") {
            ...
            // if needs writing to external js file
        ...
        if (is_numeric($this->map_width)) {
            // if no width type set
            $this->map_width = $this->map_width . 'px';
        }
        if (is_numeric($this->map_height)) {
            // if no height type set
        ...
        return ['js' => $this->output_js, 'html' => $this->output_html, 'markers' => $this->markersInfo];
    }
    ...
    function is_lat_long($input) {
        ...
        if (count($input) == 2) {
            if (is_numeric($input[0]) && is_numeric($input[1])) {
                ...
                // not a lat long - incorrect values
                return false;
            }
        } else {
            // not a lat long - too many parts
            return false;
        }
    }
    ...
    function get_lat_long_from_address($address, $attempts = 0) {
        ...
        if ($this->geocodeCaching) {
            // if caching of geocode requests is activated
        ...
        if ($data->status == "OK") {
            ...
            if ($this->geocodeCaching) {
                // if we to need to cache this result
