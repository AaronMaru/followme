    ...
    public function pie_chart($condition = null) {
        ...
        if ($query) {
            foreach ($query as $row) {
                $this->gcharts->DataTable('country')->addRow([$row['provin'], rand(0, $row['cust'])]);
            }
        }
    ...
    public function geo_chart($condition = null) {
        ...
        if ($condition == 't') {
            ...
            if (count($query)) {
                ...
                $colorAxis = $this->gcharts->colorAxis()->colors(['blue', 'orange']);

                foreach ($query as $row) {
                    $this->gcharts->DataTable('Debt')->addRows([
                        [$row['provin'], $row['cust']]
                    ]);
                }
            }
            ...
            if (count($query)) {
                ...
                $colorAxis = $this->gcharts->colorAxis()->colors(['Green', 'Yellow']);
